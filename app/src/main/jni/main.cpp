...
//        pts[i].y = result_approx[i].y ;
    ...
    LOGD("9", "1");
    Point2f dst_pts[4] = {
            Point2f(0,0), Point2f(maxWidth - 1,0), Point2f(maxWidth-1, maxHeight-1), Point2f(0, maxHeight - 1)
    };

    LOGD("10", "1");
    Mat perspect_mat = getPerspectiveTransform(result_pts, dst_pts);
    warpPerspective(output, output, perspect_mat, Size((int(maxWidth)), (int(maxHeight))), INTER_CUBIC);
